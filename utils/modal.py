...
class Modal(ui.Modal):
    def __init__(self, role: Role, channel: TextChannel, message: str = None):
        ...
        components = [
            ui.TextInput(label="Titolo",
                         placeholder="Inserisci qua il titolo.",
                         custom_id="title",
                         style=TextInputStyle.short,
                         required=False),
            ui.TextInput(label="Testo",
    ...
    async def callback(self, inter: ModalInteraction):
        try:
            if self.message_id is None:
                embed = Embed(title=inter.text_values["title"],
                              description=inter.text_values["description"],
                              color=0xffd519)
                await self.channel.send("<@&" + str(self.role.id) + ">",
                                        embed=embed)
            else:
                ...
                embed = Embed(
                    title=old_embed.title if inter.text_values["title"] == ""
                    else inter.text_values["title"],
                    description=old_embed.description
                    if inter.text_values["description"] == "" else
                    inter.text_values["description"],
                    color=0xffd519)
                await message.edit(content="<@&" + str(self.role.id) + ">",
                                   embed=embed)
            ...
            await send_response_and_clear(inter, False,
                                          "ID del messaggio non corretto.")
        except Exception as error:
            await inter.response.send_message(
                "Errore durante la generazione del modal.\n" + str(error))
